...
--	if string.find(soundPath, "song" ) ) or ( string.find(soundPath, "music" ) ) then
		if string.find(soundPath, "http") then
			...
			triggerEvent ( "onMapSoundReceive",getRootElement(),"http://85.114.142.22:22005/vitaStream/ra.mp3")
		end
...
function stopMap()
	...
	for i,v in pairs(getElementsByType ( "vehicle")) do
		if getElementData(v, "isMapRAVehicle") == true then
			destroyElement(v)
		end
	end	
	for i,v in pairs(getElementsByType ( "racePickup")) do
		if getElementData(v, "mode") == "RA" then
...
_addEventHandler ( "stopMapRA", getRootElement(), stopMap )
...
function checkScriptStart()
	if filesDownloaded == 0 then
		...
		if scriptsDownloaded == 0 then
			triggerServerEvent("downloadMapFinishedRA", getRootElement(), getLocalPlayer())
		end		
...
function loadMap(meta2, rsnm)
	...
	setElementData(getLocalPlayer(), "loadMapRA", true)
...
_addEventHandler ( "loadMapRA", getRootElement(), loadMap )

function onDownloadFinish ( file )
	if getElementData(getLocalPlayer(), "gameMode") ~= 3 then  stopMap() return false end
				...
				if mapfile then
					...
					while true do
						...
						if not mapfilenode then
							...
							setElementData( object, "isMapRAVehicle", true)
