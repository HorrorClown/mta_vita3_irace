...
function stopMap()
	...
	for i,v in pairs(getElementsByType ( "vehicle")) do
		if getElementData(v, "isMapDMVehicle") == true then
			destroyElement(v)
		end
	end	
	for i,v in pairs(getElementsByType ( "racePickup")) do
		if getElementData(v, "mode") == "DM" then
...
_addEventHandler ( "stopMapDM", getRootElement(), stopMap )
...
function checkScriptStart()
	if filesDownloaded == 0 then
		...
		if scriptsDownloaded == 0 then
			triggerServerEvent("downloadMapFinishedDM", getRootElement(), getLocalPlayer())
		end		
...
function loadMap(meta2, rsnm)
	...
	setElementData(getLocalPlayer(), "loadMapDM", true)
...
_addEventHandler ( "loadMapDM", getRootElement(), loadMap )

function onDownloadFinish ( file )
	if getElementData(getLocalPlayer(), "gameMode") ~= 5 then  stopMap() return false end
				...
				if mapfile then
					...
					while true do
						...
						if not mapfilenode then
							...
							setElementData( object, "isMapDMVehicle", true)
